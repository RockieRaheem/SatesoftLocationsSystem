...
const EditShopModal: React.FC<EditShopModalProps> = ({ isOpen, onClose, onUpdate, shopToEdit, theme, countries, users }) => {
    const [step, setStep] = useState(1);
    const [isClosing, setIsClosing] = useState(false);
    const [shopData, setShopData] = useState<Partial<Shop>>(shopToEdit || {});

    useEffect(() => {
        if (isOpen && shopToEdit) {
            setShopData(shopToEdit);
        }
    }, [isOpen, shopToEdit]);
    ...
    if (!isOpen && !isClosing || !shopToEdit) return null;
    ...
    return (
        <div className={`fixed inset-0 bg-black z-50 flex justify-center items-center p-4 transition-opacity duration-300 ${isOpen && !isClosing ? 'opacity-100 bg-opacity-50' : 'opacity-0'}`} aria-modal="true" role="dialog">
            <div className={`${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'} rounded-lg shadow-xl w-full max-w-4xl max-h-[95vh] flex flex-col transition-all duration-300 ease-in-out ${isOpen && !isClosing ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                <div className={`flex justify-between items-center p-6 border-b ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                    <h2 className={`text-2xl font-semibold ${theme === 'dark' ? 'text-slate-100' : 'text-slate-900'}`}>Edit {shopToEdit.name}</h2>
                ...
                <div className={`flex justify-between items-center p-6 border-t rounded-b-lg ${theme === 'dark' ? 'border-slate-700 bg-black/50' : 'border-slate-100'}`}>
                    <button onClick={handleBack} disabled={step === 1} className="px-6 py-2.5 text-sm font-medium border rounded-md shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                        Back
                    </button>
                    {step < 3 ? (
                        ...
                        <button onClick={handleUpdate} className="ml-3 px-6 py-2.5 text-sm font-semibold text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700">
                            Update Shop
                        </button>
                    )}
                </div>
...
const Step1 = ({ data, setData, theme, countries, users }: { data: any, setData: Function, theme: Theme, countries: Country[], users: (ShopUser|SuperUser)[] }) => {
    // ... (Step 1 implementation same as existing, just re-pasting to keep context in file update)
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Select label="Owner" name="ownerId" value={data.ownerId || ''} onChange={(e) => setData({ ...data, ownerId: parseInt(e.target.value, 10) })} theme={theme}>
                    <option value="">Select an owner</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.name} ({u.userType})</option>)}
                </Select>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             ...
             <div className={`pt-4 mt-4 border-t ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                ...
                {data.isRegistered && (
                    <div className={`p-4 mt-4 border rounded-lg space-y-4 ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                        ...
                        <div className={`pt-4 mt-4 border-t ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                            <h3 className="font-semibold mb-2">Licenses and certificates</h3>
                            <div className="space-y-4">
                                {data.tradingLicenses?.map((license: any, index: number) => {
                                    ...
                                    return (
                                        <div key={index} className={`p-4 border rounded-lg relative space-y-4 ${theme === 'dark' ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'}`}>
                                            <div className="flex justify-between items-start">
                                                <span className={`px-2 py-1 text-xs font-semibold rounded-full inline-block ${statusColor}`}>{statusText}</span>
                                                <button onClick={() => handleRemoveLicense(index)} className={`p-1 rounded-full ${theme === 'dark' ? 'text-slate-500 hover:bg-slate-700 hover:text-slate-300' : 'text-slate-400 hover:bg-slate-200 hover:text-slate-600'}`}>
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <textarea 
                                                    ...
                                                    className={`mt-1 block w-full rounded-md shadow-sm sm:text-sm px-3 py-2 border ${commonInputClasses(theme)} ${commonFocusClasses}`}
                                                    rows={2}
                                 ...
                                 <button type="button" onClick={handleAddLicense} className={`w-full flex items-center justify-center space-x-2 px-4 py-2 text-sm font-medium border-2 border-dashed rounded-md transition-colors ${theme === 'dark' ? 'border-slate-700 text-slate-400 hover:bg-slate-800 hover:border-slate-600' : 'border-slate-300 text-slate-500 hover:bg-slate-100 hover:border-slate-400'}`}>
...
const Step2 = ({ data, setData, theme }: { data: any, setData: Function, theme: Theme }) => {
    ...
    const handleLoanChange = (field: string, value: any) => {
        ...
        const updatedLoanDetails = {
            ...(data.financials?.loanDetails || { amount: 0, interest: 0, totalLoan: 0, paidLoan: 0, loanBalance: 0, startDate: '', endDate: '', issuer: '', dueDays: 0 }),
            [field]: val
        };
        ...
        // Calculate due days if dates are present
        ...
        updatedLoanDetails.loanBalance = (updatedLoanDetails.totalLoan || 0) - (updatedLoanDetails.paidLoan || 0);
        ...
        <div className="space-y-6">
             <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <Input 
                        label="Cash at hand" 
                        value={formatNumber(data.financials?.openingBalances?.cashAtHand || 0)} 
                    ...
                    <Input 
                        ...
                        value={formatNumber(data.financials?.openingBalances?.outstandingDebt || 0)} 
                    ...
                    <Input 
                        ...
                        value={formatNumber(data.financials?.openingBalances?.creditsReceivables || 0)} 
                ...
                <div className="space-y-4">
                    {/* Row 1: Issuer, Loan Amount, Interest */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <Input 
                            label="Loan Amount" 
                            value={formatNumber(data.financials?.loanDetails?.amount || 0)} 
                        ...
                        <Input 
                            label="Interest (%)" 
                            type="text"
                            value={(data.financials?.loanDetails?.interest || 0) + '%'} 
                            onChange={e => {
                    ...
                    {/* Row 2: Total Loan, Paid Loan, Loan Balance */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <Input 
                            label="Total Loan" 
                            value={formatNumber(data.financials?.loanDetails?.totalLoan || 0)} 
                            readOnly
                            theme={theme} 
                            alignRight 
                        />
                    ...
                    {/* Row 3: Start Issue Date, End Date, Due Period */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <Input 
...
const Step3 = ({ data, setData, theme }: { data: any, setData: Function, theme: Theme }) => (
    <div className="space-y-4">
        ...
        <h4 className="text-md font-semibold mt-4">Discounting Settings</h4>
        ...
        {data.isRegistered && (
             <Toggle label="VAT Registered" description="Is this shop registered for VAT collection?" enabled={data.settings?.isVatRegistered} setEnabled={(val: boolean) => setData({ ...data, settings: { ...data.settings, isVatRegistered: val } })} theme={theme} />
        )}

        <h4 className="text-md font-semibold mt-4">Payment Methods</h4>
        ...
        <h4 className="text-md font-semibold mt-4">Wallet Configuration</h4>
...
const Select = ({ label, theme, children, ...props }: { label: string, theme: Theme, children: React.ReactNode } & React.SelectHTMLAttributes<HTMLSelectElement>) => (
    <div>
        <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>{label}</label>
...
const Toggle: React.FC<{ label: string, description?: string, enabled: boolean, setEnabled: (enabled: boolean) => void, theme: Theme }> = ({ label, description, enabled, setEnabled, theme }) => (
    <div className={`p-4 rounded-lg flex items-center justify-between ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'}`}>
        <div>
            <p className={`font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-900'}`}>{label}</p>
            {description && <p className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>{description}</p>}
        </div>
        <button type="button" onClick={() => setEnabled(!enabled)} role="switch" aria-checked={enabled} className={`relative inline-flex items-center h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-yellow-500 ${enabled ? 'bg-yellow-500' : (theme === 'dark' ? 'bg-slate-600' : 'bg-slate-200')}`}>
...
const FileInput: React.FC<{ label: string, theme: Theme, onFileSelect: (base64: string | null) => void }> = ({ label, theme, onFileSelect }) => {
    ...
    return (
        <div>
            <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>{label}</label>
            <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 rounded-md ${theme === 'dark' ? 'border-slate-700 border-dashed' : 'border-slate-300 border-dashed'}`}>
                <div className="space-y-1 text-center">
                    <Icon name="upload" className="mx-auto h-12 w-12 text-gray-400" />
                    <div className="flex text-sm text-gray-600">
                        <label htmlFor={label.replace(' ', '-')} className="relative cursor-pointer rounded-md font-medium text-yellow-500 hover:text-yellow-600">
