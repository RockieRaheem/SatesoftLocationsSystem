...
const DeleteSupplierModal: React.FC<DeleteSupplierModalProps> = ({ isOpen, onClose, onConfirm, supplier, theme }) => {
            ...
            <div className={`w-full max-w-md rounded-lg shadow-xl flex flex-col transform transition-all duration-300 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'} ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
                ...
                <div className="p-6 space-y-4">
                    <p className={`text-sm ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
                        Are you sure you want to delete <strong>{supplier.companyName}</strong>? This action moves the supplier to the trash.
                    </p>
                    ...
                    <div>
                        ...
                        <textarea 
                            ...
                            placeholder="Why are you deleting this supplier?"
                        />
                    </div>
