...
const MainContent: React.FC<MainContentProps> = props => {
  ...
  return <main className="min-w-0 flex-1 overflow-y-auto bg-[#f7f7f3] text-[#1f1f1f]">
    ...
    <div className="mx-auto max-w-[1500px] px-4 py-7 sm:px-7 lg:px-10 lg:py-9"><p className="mb-7 max-w-2xl text-sm leading-6 text-[#77766f]">{meta.description}</p><Suspense fallback={<div className="grid min-h-[320px] place-items-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#dedcd4] border-t-[#1f1f1f]" /></div>}>{render()}</Suspense></div>
  </main>;
};
