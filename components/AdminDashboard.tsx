...
const AdminDashboard: React.FC = () => {
  ...
  return <FirebaseErrorBoundary theme={theme}><div className="flex h-screen overflow-hidden"><Sidebar theme={theme} activeView={activeView} isOpen={sidebarOpen} user={user} onNavigate={setActiveView} onClose={() => setSidebarOpen(false)} onToggleTheme={() => setTheme(current => current === 'dark' ? 'light' : 'dark')} onLogout={logout} /><MainContent theme={theme} activeView={activeView} user={user} countries={countries} regionalLevels={regionalLevels} onToggleSidebar={() => setSidebarOpen(true)} onNavigate={setActiveView} onAddCountry={addCountry} onUpdateCountry={updateCountry} onDeleteCountry={removeCountry} onSaveRegionalLevel={saveRegion} onDeleteRegionalLevel={removeRegion} /></div></FirebaseErrorBoundary>;
};
