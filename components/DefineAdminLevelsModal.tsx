...
import { Country, Theme, AdminLevelName } from '../types';
import Icon from './Icon';

interface DefineAdminLevelsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (countryCode: string, levels: AdminLevelName[], count: number) => void;
  theme: Theme;
  countryToDefine: Country | null;
}

const DefineAdminLevelsModal: React.FC<DefineAdminLevelsModalProps> = ({ isOpen, onClose, onSave, theme, countryToDefine }) => {
    const [isClosing, setIsClosing] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [levels, setLevels] = useState<Omit<AdminLevelName, 'level'>[]>([]);
    const [levelCount, setLevelCount] = useState<number>(1);

    useEffect(() => {
        if (isOpen && countryToDefine) {
            const initialCount = countryToDefine.numberOfAdminLevels || 1;
            setLevelCount(initialCount);

            const existingNames = countryToDefine.adminLevelNames || [];
            ...
            <div className={`${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'} rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col transition-all duration-300 ease-in-out ${isOpen && !isClosing ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                <div className={`flex justify-between items-center p-6 border-b ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                    <h2 className={`text-2xl font-semibold ${theme === 'dark' ? 'text-slate-100' : 'text-slate-900'}`}>
                        Define Levels for {countryToDefine.name}
                    </h2>
                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                    ...
                    <div>
                        <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>Level Names (from top-level down)</label>
...
export default DefineAdminLevelsModal;