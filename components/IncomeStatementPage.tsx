...
const IncomeStatementPage: React.FC<IncomeStatementPageProps> = ({ theme, onViewStatement }) => {
    return (
            ...
            <div className="overflow-x-auto">
                <table className={`min-w-full divide-y ${theme === 'dark' ? 'divide-slate-700' : 'divide-slate-200'}`}>
                    <thead className={theme === 'dark' ? 'bg-slate-800' : 'bg-slate-50'}>
                        <tr>
                            {['Shop Name', 'Country', 'Revenue', 'Gross Profit', 'Operating Income', 'Net Income', 'Actions'].map((header) => (
                    ...
                    <tbody className={`divide-y ${theme === 'dark' ? 'bg-slate-900 divide-slate-700' : 'bg-white divide-slate-200'}`}>
                        {mockShops.map((shop) => (
                            <tr key={shop.id} className={theme === 'dark' ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}>
                                ...
                                <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>{shop.financials?.incomeStatement?.revenue?.toLocaleString() || '0'}</td>
                                <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>{shop.financials?.incomeStatement?.grossProfit?.toLocaleString() || '0'}</td>
                                <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>{shop.financials?.incomeStatement?.operatingIncome?.toLocaleString() || '0'}</td>
                                <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold text-right ${theme === 'dark' ? 'text-slate-100' : 'text-slate-800'}`}>{shop.financials?.incomeStatement?.netIncome?.toLocaleString() || '0'}</td>
